...
async function run() {
	...
	for await (const line of rl) {
		...
		if (trimmed.startsWith("INSERT INTO `")) {
			...
			if (match) {
				...
				if (
					currentTable === "claimdata" ||
					currentTable === "specialclaimdata"
				) {
					...
					currentTable === "customercompanyitemwisesalespara" ||
					currentTable === "customer_company_itemwise_sales_para"
				) {
	...
	console.log(
		`Extracted ${customersMap.size} unique customers from various tables.`,
	);
	...
	if (batch.length > 0) {
		...
		for (let i = 0; i < batch.length; i += 1000) {
			...
			const values = chunk
				.map(
					(c) =>
						`('${c.id.replace(/'/g, "''")}', '${c.name.replace(/'/g, "''")}', '${c.city.replace(/'/g, "''")}')`,
				)
				.join(",");
