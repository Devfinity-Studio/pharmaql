...
async function run() {
	...
	console.log(`Found ${assignmentsToAdd.length} unique MR assignments in SQL dumps.`);
	...
	for (const assignment of assignmentsToAdd) {
		...
		if (existing.length === 0) {
			console.log(`Adding missing assignment: ${assignment.mrId} -> ${assignment.manufacturer} / ${assignment.division}`);
			await db.insert(mrManufacturers).values({
				id: `${assignment.mrId}-${assignment.manufacturer}-${assignment.division}`.substring(0, 50),
