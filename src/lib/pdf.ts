...
export function generateStockPdfReport(
  ...
  const divisions = [
    ...new Set(
      data.map((item) => item.Division || item.Manufacturer || "UNKNOWN"),
    ),
  ].sort();
  ...
  for (const div of divisions) {
    ...
    autoTable(doc, {
      ...
      didParseCell: (data) => {
        ...
        if (data.section === "body" && (data.row.raw as any).isTotal) {
          data.cell.styles.fontStyle = "bold";
          if (data.column.dataKey === "Item Name") {
  ...
  autoTable(doc, {
    ...
    didParseCell: (data) => {
      ...
      if (data.section === "body") {
        data.cell.styles.fontStyle = "bold";
        if ((data.row.raw as any).isGrandTotalRow) {
          data.cell.styles.fontSize = 8;
          data.cell.styles.fillColor = [243, 244, 246];
        } else if ((data.row.raw as any).isSummaryRow) {
    ...
    willDrawCell: (data) => {
      if (data.section === "body") {
        if ((data.row.raw as any).isSummaryRow) {
          doc.setDrawColor(200, 200, 200);
        ...
        } else if ((data.row.raw as any).isGrandTotalRow) {
          doc.setDrawColor(11, 37, 69);
...
export function generateSalesPdfReport(
  ...
  for (const div of divisions) {
    ...
    const customers = [
      ...new Set(divData.map((d) => d.Customer || "Unknown Party")),
    ].sort();
    ...
    for (const cust of customers) {
      const custData = divData.filter(
        (d) => (d.Customer || "Unknown Party") === cust,
      );
...
export function generateFreeSchemePdfReport(
  ...
  doc.setFontSize(8);
  doc.setFont("helvetica", "bold");
  doc.text("Qty Claim : Claim Value = PTR x ClaimQty", legendRightX, legendY, {
    align: "right",
  });
  legendY += 4;
  doc.text(
  ...
  for (const div of divisions) {
    ...
    const claimTypes = [
      ...new Set(divData.map((d) => d.SchemeType || "Qty")),
    ].sort();
    ...
    for (const cType of claimTypes) {
      ...
      const customers = [
        ...new Set(
          typeData.map((d) => d.Party || d.Customer || "Unknown Party"),
        ),
      ].sort();
      ...
      typeBodyData.push([
        ...
        {
          content: typeSaleQty > 0 ? typeSaleQty.toString() : "-",
          styles: { fontStyle: "bold", halign: "right" },
        },
        {
          content: typeFreeQty > 0 ? typeFreeQty.toString() : "-",
          styles: { fontStyle: "bold", halign: "right" },
        },
        {
          content: typeActualFQty > 0 ? typeActualFQty.toString() : "-",
          styles: { fontStyle: "bold", halign: "right" },
        },
        {
          content: typeClaimQty > 0 ? typeClaimQty.toString() : "-",
          styles: { fontStyle: "bold", halign: "right" },
        },
        { content: "", styles: { fontStyle: "bold" } },
        {
          content: typeClaimValue > 0 ? typeClaimValue.toFixed(2) : "-",
          styles: { fontStyle: "bold", halign: "right" },
        },
        { content: "", colSpan: 2 },
      ]);
      ...
      Array.from(typeSummaryMap.keys())
        .sort()
        .forEach((pName) => {
