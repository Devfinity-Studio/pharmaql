...
- **Implementation:**
  ...
  - **Full Sync Mode:** The CSV's `StockQty` will completely _replace_ the existing stock value in the database.
  - **Incremental Mode:** The CSV's `StockQty` will be _added_ to the existing stock value.
  - _Note on Sales:_ Sales records (`SaleQty`) will always be treated as incremental (appended as new records).
...
- _Rationale:_ As requested, once the basic entities are auto-created by the ingestion, the Admin can manually "feed the remaining fields" later via the Admin Dashboard.
...
- _Future Scope:_ If mathematical automation is required during order placement later, the string parsing logic will be updated, but for now, it simply acts as an informational label for the MRs.
